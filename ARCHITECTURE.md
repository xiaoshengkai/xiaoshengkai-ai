...
└── packages/
    ...
    └── workflows/               # 工作流引擎（@app/workflows，详见「工作流系统」）
        ...
        └── templates/
            ...
            └── floorplan-remodel/# 户型改造（2 步：parse/generate；视觉识别结构→CV 像素吸附→LLM 梯度方案→原图 1:1 叠层 SVG 红=砸墙蓝=砌墙）
                ├── actions.json # 模板级动作（floorplan-upload）
                └── lib/         # parse（视觉+sharp 墙像素 mask 吸附+quality.json）/ generate（承重硬校验+可拆白名单+按方案判定不回退）/ render（原图 base64 叠层+编号+徽章）/ snap（墙像素 mask/吸附/外围 bbox）
```
