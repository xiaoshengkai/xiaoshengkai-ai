...
### 变更
- **三档模式切换**：聊天新增 `chat`（纯聊，绿）/ `plan`（只读，只出方案，橙）/ `build`（全权，蓝）三种模式，输入框左侧循环按钮切换（按钮底色即当前模式色），每对话独立记忆（存 `data/conversations/*.json` 的 `mode` 字段，缺省回 chat 向后兼容，旧记录 `edit` 自动映射为 `build`）。
- **写工具权限**（`lib/modes.ts` 新增，`WRITE_TOOLS` 11 个：exec + 文件写 6 + 知识库写 4）：非 build 模式剥离这些写工具，只保留只读/搜索/知识库检索/生图/图表；plan 模式额外注入「只读分析、只输出方案」system prompt 引导。
...
### 验证
- typecheck + `test:mm` 18 项全绿（新增 `modes.test.ts` 5 项：build 保留原对象 / chat、plan 剥离写工具 / WRITE_TOOLS 完整性；`conversations-store.test.ts` 加 mode 持久化 + 缺省默认 chat 2 项）
