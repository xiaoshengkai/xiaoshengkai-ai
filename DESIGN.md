...
| 边框 | 界面统一 2px 纯黑实线 + 零模糊纯黑硬阴影（`Npx Npx 0 #000`）；聊天气泡/输入区保留 3px 签名重量 |
| 阴影分级 | 静态面 2px（`--shadow-1`）/ hover·浮空 4px（`--shadow-2`）/ 浮层 6px（`--shadow-3`） |
| 主色 | 电光黄 `#FFE135`（黑字），粉/蓝/紫/橙/深绿点缀 |
| 气泡 | 用户黑底白字 + 黄硬阴影（`bg-ink`）、AI 白底黑字（`bg-card`），3px 黑边 |
