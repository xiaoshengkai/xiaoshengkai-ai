...
function svgWrap(W, H, title, lines, body, img) {
  const strip = 46 + lines.length * 24;
  const total = H + strip;
  const base = img
    ? `<image href="data:${img.mime};base64,${img.base64}" width="${W}" height="${H}" preserveAspectRatio="none"/>`
...
function openingEls(structure, gapW) {
  ...
  for (const o of structure.openings || []) {
    ...
    els.push(o.type === "window"
      ? lineEl(gx1, gy1, gx2, gy2, C.window, gapW * 0.4)
      : `<path d="M ${gx2} ${gy2} A ${half * 2} ${half * 2} 0 0 1 ${gx2 - uy * half * 2} ${gy2 + ux * half * 2}" fill="none" stroke="${C.door}" stroke-width="${gapW * 0.3}"/>`);
  }
  return els.join("\n");
}
...
export function renderStructure(structure, img = null) {
  ...
  let body;
  if (img) {
    const bearing = (structure.walls || []).filter(w => w.bearing)
      .map(w => lineEl(w.x1, w.y1, w.x2, w.y2, C.bearing, strokeW, 0.8)).join("\n");
    const align = (structure.walls || []).filter(w => !w.unverified)
      .map(w => lineEl(w.x1, w.y1, w.x2, w.y2, C.align, strokeW * 0.45, 0.95)).join("\n");
    const unverified = (structure.walls || []).filter(w => w.unverified)
      .map(w => `<line x1="${w.x1}" y1="${w.y1}" x2="${w.x2}" y2="${w.y2}" stroke="${C.door}" stroke-width="${strokeW * 0.4}" stroke-dasharray="8 8" stroke-opacity="0.6"/>`).join("\n");
    body = `${bearing}\n${align}\n${unverified}`;
  } else {
  ...
  const lines = wrapLines("图例：黄线=识别墙位 红=承重墙不可拆；请核对线与原图墙体是否贴合，有误请重试", chunk);
  return svgWrap(W, H, "结构识别预览", lines, body, img);
}
...
export function renderPlan(structure, plan, img = null, extraLines = []) {
  ...
  if (img) {
    const demolish = (plan.demolish || []).map(id => {
      ...
      return w ? lineEl(w.x1, w.y1, w.x2, w.y2, C.demolish, strokeW * 1.2, 0.85) : "";
    }).filter(Boolean).join("\n");
    body = `${demolish}\n${newRoomEls(plan, W)}\n${buildEls(plan, strokeW)}\n${badgeEls(structure, plan)}`;
  } else {
    const base = (structure.walls || []).map(w => lineEl(w.x1, w.y1, w.x2, w.y2, C.dim, strokeW)).join("\n");
    const demolish = (plan.demolish || []).map(id => {
      const w = wallById(structure, id);
      return w ? lineEl(w.x1, w.y1, w.x2, w.y2, C.demolish, strokeW * 1.2) : "";
    }).filter(Boolean).join("\n");
    body = `${base}\n${openingEls(structure, strokeW)}\n${demolish}\n${newRoomEls(plan, W)}\n${buildEls(plan, strokeW)}\n${roomEls(structure, plan.roomChanges)}`;
  }
  const chunk = Math.floor(W / 17);
  const checks = (plan.checks || []).map(c => `${c.pass ? "✓" : "✗"}${c.rule}${c.note && !c.pass ? `(${c.note})` : ""}`).join(" ");
  const lines = ["图例：色块=新增功能空间 蓝线=新砌墙 红=砸墙 | 示意方案，施工前需专业鉴定",
    ...wrapLines(`规范自检：${checks}`, chunk),
    ...extraLines.flatMap(l => wrapLines(l, chunk))];
  return svgWrap(W, H, plan.title || plan.id, lines, body, img);
}
...
export function demo() {
  ...
  const plan = {
    ...
    newRooms: [{ label: "功能房", bbox: [500, 350, 950, 650] }],
    roomChanges: [{ roomId: "r2", newLabel: "功能房" }],
    checks: [{ rule: "承重结构未破坏", pass: true }, { rule: "新马桶间邻近排水", pass: false, note: "需提升泵" }],
  };
  const img = { base64: TINY_PNG, mime: "image/png" };
  const sImg = renderStructure(structure, img);
  const pImg = renderPlan(structure, plan, img, ["丢弃：方案X 试图拆承重墙"]);
  const sAbs = renderStructure(structure);
  const pAbs = renderPlan(structure, plan);
  const assert = (cond, msg) => { if (!cond) throw new Error(`render self-check failed: ${msg}`); };
  assert(sImg.includes("<image href="), "img mode embeds base image");
  assert(sImg.includes(C.align), "img mode has align overlay");
  assert(sImg.includes(C.bearing), "img mode has bearing overlay");
  assert(pImg.includes(C.demolish), "plan img has demolish red");
  assert(pImg.includes(PALETTE[0]), "plan img has newRoom fill");
  assert(pImg.includes(">功能房</text>"), "plan img has newRoom label");
  assert(pImg.includes("→ 功能房"), "plan img has rename badge");
  assert(pImg.includes("丢弃：方案X"), "plan img shows discarded reason");
  assert(pImg.includes("施工前需专业鉴定"), "disclaimer present");
  assert(sAbs.includes(C.bearing) && sAbs.includes("承重"), "abstract fallback ok");
  assert(pAbs.includes(PALETTE[0]), "abstract fallback fill");
